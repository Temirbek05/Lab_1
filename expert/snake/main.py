...
def game_loop(username):
    global snake_position, snake_body, direction, score
    ...
    while running:
        ...
        for food in foods[:]:
            ...
            pygame.draw.rect(game_window, food["color"], pygame.Rect(food["position"][0], food["position"][1], SIZE, SIZE))
            print(f"Drawing food: {food['type']} at {food['position']} with timer {food['timer']}")  # Debug statement
            food["timer"] -= 1
            if food["timer"] <= 0:
                print(f"Removing food: {food['type']} at {food['position']} due to timer expiry")  # Debug statement
                foods.remove(food)
        ...
        # Game over conditions (collision checks)
        ...
        for x, y in wall.body:
            if (snake_position[0], snake_position[1]) == (x * SIZE, y * SIZE):
                running = False
                break
